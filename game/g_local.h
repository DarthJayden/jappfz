...
struct gentity_s {
	...
	int			attached; //JAYDEN: ATTACHED TO OBJECT. Это для изменения положения эффекта снаряда при изменении положения снаряда. 
	zmissle_t			attached_missle; //Энтити запускает Фаербол! Тут хранятся все данные о нём, кроме анимации запуска (таймеры там же!!). Она в функции.
};
...
typedef struct clientPersistant_s {
	...
	int	                newMaxHealth; //Jayden: kostil
	int                 newMaxForce; //AGAIN JAYDEN
	int					newMaxArmor; //Stop dat, Jay!!
	...
	//Jayden
	char				clanpass[MAX_STRING_CHARS]; //Clanpass, как ни странно. No need of that, actually, but... I need this.
	qboolean            RP_mod; //For RP chat-mods.
	char				modelname_allowed[MAX_QPATH]; //allowed models, default and custom for this character (account).
	//char				sabers_allowed[MAX_QPATH]; 
	allowed_sabers  sabers_allowed[]; //alloed sabers (non default)
	// / Jayden
