...
#define SABER_NONATTACK_DAMAGE		2
		 ...
		 //turn the parry into a knockaway
			...
			if (!PM_SaberInBounce(self->client->ps.saberMove) &&
				...
				//	defendStr += Q_irand(0, otherOwner->client->saber[0].parryBonus);
					defendStr += otherOwner->client->saber[0].parryBonus;
				if (otherOwner->client->saber[1].model[0]
					...
					defendStr += otherOwner->client->saber[1].parryBonus;
				}
				...
				attackBonus = /*Q_irand(0, */self->client->saber[0].breakParryBonus / 2/*) Math expected*/;
				if (self->client->saber[1].model[0]
					&& !self->client->ps.saberHolstered) {
					attackBonus += /*Q_irand(0, */self->client->saber[1].breakParryBonus / 2/* ---//---)*/;
				}
		...
		if (!grappler->inuse || !grappler->client || grappler->client->grappleIndex != self->s.number ||
			...
			if (VectorLength(&grapAng) > 64.0f) { //too far away, break it off
				...
				if (self->client->grappleState >= 20) { //grapplee
						...
						if (self->client->grappleState == 1) { //knee to the face
							...
							if (self->client->ps.torsoTimer < 1000) {
								...
								if (grappler->health > 0) {
									//Jayden
									grappler->client->ps.forceHandExtend = HANDEXTEND_KNOCKDOWN;
									grappler->client->ps.forceHandExtendTime = level.time + 3300;
									if (grappler->client->ps.forceHandExtendTime > level.time)
									{
										grappler->client->ps.speed = 0;
										grappler->client->ps.basespeed = 0;
										ucmd->forwardmove = 0;
										ucmd->rightmove = 0;
										ucmd->upmove = 0;
									}
					...
					//Jayden
					else if (self->client->ps.torsoAnim == BOTH_KYLE_PA_3) { //well, third grab
						if (self->client->grappleState == 1) { //Khhr!!1
							if (self->client->ps.torsoTimer < 3200) {
								int grapplerAnim = grappler->client->ps.torsoAnim;
								int grapplerTime = grappler->client->ps.torsoTimer;
