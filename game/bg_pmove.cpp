...
static void PM_Footsteps( void ) {
	...
	if ( pm->ps->saberMove == LS_SPINATTACK ) {
		...
		if ( PM_RunningAnim( pm->ps->legsAnim ) ) {
			const qboolean gripRoll = !!(JP_GetJPFixRoll() & 1) || !!(JP_GetJPFixRoll() & 4);
			const qboolean continuousRoll = !!(JP_GetJPFixRoll() & 2) || !!(JP_GetJPFixRoll() & 4);
...
static void PM_Weapon( void ) {
	...
	if ( pm_entSelf->s.NPC_class != CLASS_VEHICLE
		...
		veh = pm_entVeh;
		if (( veh && veh->m_pVehicle && (veh->m_pVehicle->m_pVehicleInfo->type == VH_WALKER || veh->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER))/*Jayden: ради... safe-zone*/ || pm->ps->powerups[PW_YSALAMIRI]) {//riding a walker/fighter
...
//	if (/*Jayden: ради... safe-zone*/ pm->ps->powerups[PW_YSALAMIRI])
	//{
	//	pm->cmd.buttons &= ~(BUTTON_ATTACK | BUTTON_ALT_ATTACK);
	//}
		...
		if ( BG_InRoll( ps, ps->legsAnim ) && ps->speed > 50.0f ) { //can't roll unless you're able to move normally
			if ( (ps->legsAnim) == BOTH_ROLL_B ) { //backwards roll is pretty fast, should also be slower
				if ( ps->legsTimer > 800 || (JP_GetJPFixRoll() & 4 && ps->legsTimer > 100) )
					ps->speed = ps->legsTimer / /*2.5f*/2.f;
				else
