...
static void Cmd_Go_to(gentity_t *ent)
{
	if (!ent) {
	...
	if (!japp_allowgoto.integer)
	{
		trap->SendServerCommand(ent->client->ps.clientNum, "NOPE! GOTO IS NOT ALLOWED BY SERVER CFG! BLAME THE ADMINISTRATION!\n");
	}
	...
	// 1 arg means we're teleporting ourself to x (self -> client, self -> namedTelePos)
		...
		// must be teleporting self -> client
		 if (g_entities[targetClient].inuse) {
		...
		trap->SendServerCommand(ent->client->ps.clientNum, "Usage: goto player's ID OR goto x y z\n use serverstatus to show IDs\nuse origin to show coordinates\n");
}
...
static void G_SayTo( gentity_t *ent, gentity_t *other, int mode, char color, const char *name, const char *message, char *locMsg ) {
	...
	//Jayden
	if (mode == SAY_CLAN && !(strcmp(other->client->pers.clanpass, ent->client->pers.clanpass) == 0)) {
		return;
	}
	// /Jayden
...
static void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText ) {
	...
	/*case SAY_ALL:
		...
		//Jayden
	case SAY_CLAN:
		G_LogPrintf(level.log.console, "Comm: %s: %s\n", ent->client->pers.netname, chatText);
		Com_sprintf(name, sizeof(name), S_COLOR_RED"<Comm>" S_COLOR_WHITE "%s%c%c" EC ": ",
			ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE
		);
		...
		break;
	case SAY_ALL:
		if (!Q_stricmpn("/shout ", chatText, 6) || (!Q_stricmpn("!", chatText, 1)) && ent->client->pers.RP_mod) {
			...
			G_LogPrintf(level.log.console, "Shout: %s shouts: %s\n", ent->client->pers.netname, chatMod);
			Com_sprintf(name, sizeof(name), "%s%c%c" EC" shouts: ", ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE);
			color = COLOR_RED;
			distance = DIST_SAY_SHOUT;
		}
		else if (!Q_stricmpn("/me ", chatText, 3) || (!Q_stricmpn("=", chatText, 1)) && ent->client->pers.RP_mod) {
			if (!Q_stricmpn("/me ", chatText, 3)) {
				chatMod = (char*)chatText + 3;
			}
			else {
				chatMod = (char*)chatText + 1;
			}
			G_LogPrintf(level.log.console, "Action: %s  %s\n", ent->client->pers.netname, chatMod);
			...
			chatMod = (char*)chatText;
			G_LogPrintf(level.log.console, "Roll Action: %s rolls  %s\n", ent->client->pers.netname, chatMod);
			Com_sprintf(name, sizeof(name), "%s%c%c" EC" ", ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE);
			color = COLOR_YELLOW;
			distance = DIST_SAY_SHOUT;
		}
		else if (!Q_stricmpn("/low", chatText, 4) || (!Q_stricmpn(".", chatText, 1)) && ent->client->pers.RP_mod) {
			...
			G_LogPrintf(level.log.console, "Say (low): %s says (low): %s\n", ent->client->pers.netname, chatMod);
			Com_sprintf(name, sizeof(name), "%s%c%c" EC " says (low): ", ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE);
			color = COLOR_WHITE;
			distance = DIST_SAY_LOW;
		}

		else if (!Q_stricmpn("/GM", chatText, 3) || (!Q_stricmpn("#", chatText, 1)) && ent->client->pers.RP_mod) {
			...
			G_LogPrintf(level.log.console, "GM %s: %s\n", "^5GM", chatMod);
			Com_sprintf(name, sizeof(name), "%s%c%c" EC ": ", "^5GM", Q_COLOR_ESCAPE, COLOR_WHITE);
			color = COLOR_WHITE;
			distance = 0;
			}
			else
			{
				chatMod = (char*)chatText;
				chatMod = (char*)chatText;
				G_LogPrintf(level.log.console, "OOC: %s: %s\n", ent->client->pers.netname, chatMod);
				Com_sprintf(name, sizeof(name), " %s%c%c" EC ": ", ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE);
				color = COLOR_GREEN;
				distance = 0;
				trap->SendServerCommand(ent->client->ps.clientNum,"cp \"You do not have rights to be a Game Master!\n\"");
			}
		}
		else if (!Q_stricmpn("/say", chatText, 4) || !Q_stricmpn("-", chatText, 1)) {
			if (!Q_stricmpn("/say", chatText, 4)) {
				chatMod = (char*)chatText + 4;
			}
			else {
				chatMod = (char*)chatText + 1;
			...
			G_LogPrintf(level.log.console, "say: %s says: %s\n", ent->client->pers.netname, chatMod);
...
void Cmd_EngageDuel_f( gentity_t *ent, bool fullforce ) {
	...
	if ( tr->fraction < 1.0f && tr->entityNum < MAX_CLIENTS ) {
		...
		{
			ent->duelFullForce = qfalse; //if we call saberduel
		}
		if ( challenged->client->ps.duelIndex == ent->s.number && challenged->client->ps.duelTime >= level.time ) {
			trap->SendServerCommand( -1, va( "print \"%s " S_COLOR_WHITE "%s %s " S_COLOR_WHITE "(%s)!\n\"",
				...
				ent->client->pers.netname, weaponData[challenged->client->pers.duelWeapon].longName ) ); //Accept duel from ''challenged''

			if (challenged->duelFullForce){ //meant we accept guy who call as to ff duel, will play ff duel

				ent->duelFullForce = qtrue;
			}
			else //means guy call as to saber duel
			{
				ent->duelFullForce = qfalse; //we dont want to use Force! Its SABER! 
			}
			...
			ent->client->ps.ammo[ent->client->ps.weapon] == challenged->client->ps.ammo[challenged->client->ps.weapon] == 999; //we cant do much with 100 default ammo you know. 
			...
			if ( challenged->client->pers.duelWeapon == WP_SABER )
	{
			...
			ent->client->ps.duelTime = challenged->client->ps.duelTime; //se ClientThinkReal about speed=0. We dont want it!
			...
			if (fullforce)
			{
				trap->SendServerCommand(challenged - g_entities, va("cp \"%s " S_COLOR_WHITE "%s ^7with ^5Full ^5Force!\n" S_COLOR_YELLOW
					"Weapon: " S_COLOR_WHITE "%s\n\"", ent->client->pers.netname,
					G_GetStringEdString("MP_SVGAME", "PLDUELCHALLENGE"), weaponData[weapon].longName)
				);
				trap->SendServerCommand(ent - g_entities, va("cp \"%s %s ^7with ^5Full ^5Force!\n " S_COLOR_YELLOW "Weapon: " S_COLOR_WHITE "%s"
...
#define EXTINFO_CHATMOD (0x0009u)
...
static void Cmd_AMInfo_f( gentity_t *ent ) {
	...
	if (extendedInfo & EXTINFO_CHATMOD) {
		trap->SendServerCommand(ent->client->ps.clientNum, "^1Chat mods:\n^2OOC: ^7ordinar chat, all player on server can see your text;\n^7SAY: type /say or -- before your text in chat. This text can be ''heared'' only by players nearby (and spectators);\n ^7LOW: type /low or ,, before your text to lower your ''voice''. This text can be ''heared'' only by characters who is really close (and spectators);\n^1SHOUT: ^7type /shout or !! bedore your text to shout. This text can be ''heared'' by characters in a huge area around you;\n^3ACTION: ^7type /me or == bedore your text to describe some action, like ''^3Jayden crossed her arms^7''\n\n");
...
static const emote_t emotes[] = {
	...
	{ "consoletype", BOTH_CONSOLE1START, BOTH_CONSOLE1, EMF_HOLD | EMF_HOLSTER },
...
void Cmd_Clanpass_f(gentity_t *ent)
{
		...
		if (trap->Argc() != 2)
		{
			trap->SendServerCommand(ent->client->ps.clientNum, va("print \"Usage: /clanpass <password>\n\""));
			return;
		}
		trap->Argv(1, password, sizeof(password)); 
		strcpy(ent->client->pers.clanpass, password);
}

void  Cmd_Clanwhois_f(gentity_t *ent)
{
	int j; 
	for (j = 0; j < MAX_CLIENTS; j++)
	{
		gentity_t *Client = g_entities + j;
		if (Client->client && Client->client->pers.connected == CON_CONNECTED && Client->inuse)
		{
			trap->SendServerCommand(ent->client->ps.clientNum, va("print \"In channel:\n\""));
			if (strcmp(ent->client->pers.clanpass, Client->client->pers.clanpass) == 0)
			{
				trap->SendServerCommand(ent->client->ps.clientNum, va("print \"%s\n\"", Client->client->pers.netname));
			}
		}
	}
	return; 
...
void Cmd_ImRPer_f(gentity_t *ent)
{
	if (ent->client->pers.RP_mod)
	{
		ent->client->pers.RP_mod = qfalse;
		trap->SendServerCommand(ent->client->ps.clientNum, va("cp \"^3You are not ^2Roleplayer ^3now^1!\n\""));
		trap->SendServerCommand(ent->client->ps.clientNum, va("print \"^3You are not ^2Roleplayer ^3now^1!\n\""));
	}
	else
	{
		ent->client->pers.RP_mod = qtrue;
		trap->SendServerCommand(ent->client->ps.clientNum, va("cp \"^3You are ^2Roleplayer ^3now^1!\n\""));
		trap->SendServerCommand(ent->client->ps.clientNum, va("print \"^3You are ^2Roleplayer ^3now^1!\n\""));
	}
...
static const command_t commands[] = {
	...
	{ "amconsoletype", Cmd_Emote_consoletype, GTB_ALL, CMDFLAG_NOINTERMISSION | CMDFLAG_ALIVE },
	...
	//Jayden
	{ "clanpass", Cmd_Clanpass_f, GTB_ALL, CMDFLAG_NOINTERMISSION },
	...
	{ "roll", Cmd_Roll_f, GTB_ALL,CMDFLAG_NOINTERMISSION },
