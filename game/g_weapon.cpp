...
//float BLASTER_SPREAD = (float)japp_e11spread.integer; //I dont like this....
//int BLASTER_VELOCITY = japp_e11velocity.integer;
//int BLASTER_DAMAGE = japp_e11damage.integer;
...
void WP_FireBlasterMissile( gentity_t *ent, vector3 *start, vector3 *dir, qboolean altFire ) {
	int velocity = japp_e11velocity.integer;
	int	damage = /*BLASTER_DAMAGE*/japp_e11damage.integer;
	gentity_t *missile;
...
static void WP_FireBlaster( gentity_t *ent, qboolean altFire ) {
	...
	if ( altFire ) {
		...
		angs.pitch += crandom() * (float)japp_e11spread.integer/*BLASTER_SPREAD*/;
		angs.yaw += crandom() * (float)japp_e11spread.integer/*BLASTER_SPREAD*/;
	}
...
static void WP_BowcasterAltFire( gentity_t *ent ) {
	int	damage = japp_bowcasterdamage.integer ;

	gentity_t *missile = CreateMissile( &muzzle, &forward, japp_bowcastervelocity.integer, 10000, ent, qfalse );
...
static void WP_BowcasterMainFire( gentity_t *ent ) {
	int			damage = japp_bowcasterdamage.integer , count;
	float		vel;
	vector3		angs, dir;
	gentity_t	*missile;
	int i;
	...
	for ( i = 0; i < count; i++ ) {
		...
		vel = japp_bowcastervelocity.integer * (crandom() * BOWCASTER_VEL_RANGE + 1.0f);
...
static void WP_RepeaterMainFire( gentity_t *ent, vector3 *dir ) {
	int	damage = /*REPEATER_DAMAGE*/japp_repeaterdamage.integer;

	gentity_t *missile = CreateMissile( &muzzle, dir, /*REPEATER_VELOCITY*/japp_repeatervelocity.integer, 10000, ent, qfalse );
...
static void WP_RepeaterAltFire( gentity_t *ent ) {
	int	damage = /*REPEATER_ALT_DAMAGE*/japp_repeateraltdamage.integer;

	gentity_t *missile = CreateMissile( &muzzle, &forward, japp_repeateraltvelocity.integer/*REPEATER_ALT_VELOCITY*/, 10000, ent, qtrue );
	...
	missile->clipmask = MASK_SHOT | CONTENTS_LIGHTSABER;
	missile->splashDamage =japp_repeateraltsplashdamage.integer/* REPEATER_ALT_SPLASH_DAMAGE*/;
	if ( level.gametype == GT_SIEGE ) {
		...
		missile->splashRadius = /*REPEATER_ALT_SPLASH_RADIUS*/japp_repeateraltsplashradius.integer;
	}
...
static void WP_FireRepeater( gentity_t *ent, qboolean altFire ) {
	...
	if ( altFire ) {
		...
		// add some slop to the alt-fire direction
		angs.pitch += crandom() * /*REPEATER_SPREAD*/japp_repeaterspread.integer;
		angs.yaw += crandom() * /*REPEATER_SPREAD*/japp_repeaterspread.integer;
...
static void WP_DEMP2_MainFire( gentity_t *ent ) {
	int	damage = /*DEMP2_DAMAGE*/japp_demp2damage.integer;

	gentity_t *missile = CreateMissile( &muzzle, &forward, /*DEMP2_VELOCITY*/japp_demp2velocity.integer, 10000, ent, qfalse );
...
static void WP_DEMP2_AltFire( gentity_t *ent ) {
	int		damage =/* DEMP2_ALT_DAMAGE*/japp_demp2altdamage.integer;
	int		count, origcount;
	float	fact;
	vector3	start, end;
	trace_t	tr;
	...
	VectorMA( &start, /*DEMP2_ALT_RANGE*/japp_demp2altrange.integer, &forward, &end );

	count = (level.time - ent->client->ps.weaponChargeTime) / /*DEMP2_CHARGE_UNIT*/japp_demp2chargeunit.integer;
	...
	missile->splashRadius = /*DEMP2_ALT_SPLASHRADIUS*/japp_demp2splashradius.integer;
...
void rocketThink( gentity_t *ent ) {
	...
	float dot, dot2, dis;
	int i;
	float vel = (ent->spawnflags & 1) ? ent->speed : /*ROCKET_VELOCITY*/japp_rocketvelocity.integer;
	...
	ent->nextthink = level.time + /*ROCKET_ALT_THINK_TIME*/japp_rocketaltthinktime.integer;	// Nothing at all spectacular happened, continue.
	return;
}
...
static void WP_FireRocket( gentity_t *ent, qboolean altFire ) {
	int	damage = /*ROCKET_DAMAGE*/japp_rocketdamage.integer;
	int	vel = /*ROCKET_VELOCITY*/japp_rocketvelocity.integer;
	int dif = 0;
	float rTime;
	gentity_t *missile;
	...
	if ( ent->client && ent->client->ps.rocketLockIndex != ENTITYNUM_NONE ) {
		...
		if ( dif >= 10 && rTime != -1 ) {
			...
			if ( missile->enemy && missile->enemy->client && missile->enemy->health > 0 && !OnSameTeam( ent, missile->enemy ) ) { //if enemy became invalid, died, or is on the same team, then don't seek it
				...
				missile->nextthink = level.time + /*ROCKET_ALT_THINK_TIME*/japp_rocketaltthinktime.integer;
			}
		}
	...
	VectorSet( &missile->r.maxs, japp_rocketsize.integer, japp_rocketsize.integer, japp_rocketsize.integer );
	VectorScale( &missile->r.maxs, -1, &missile->r.mins );
	...
	missile->clipmask = MASK_SHOT;
	missile->splashDamage = /*ROCKET_SPLASH_DAMAGE*/japp_rocketsplashdamage.integer;
	missile->splashRadius = /*ROCKET_SPLASH_RADIUS*/japp_rocketsplashradius.integer;
