...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	/*	if ( client->ps.stats[STAT_ARMOR] > client->ps.stats[STAT_MAX_HEALTH] ) {
		...
		//Jayden
		if (client->ps.stats[STAT_ARMOR] > client->pers.newMaxArmor && client->pers.newMaxArmor) {
			client->ps.stats[STAT_ARMOR]--;
		}
	}
}
...
void ClientEvents( gentity_t *ent, int oldEventSequence ) {
	...
	for ( i = oldEventSequence; i < client->ps.eventSequence; i++ ) {
		...
		case EV_ROLL:
		{
			...
			break;
		case EV_FIRE_WEAPON:
			//Jayden FOR SAFE-ZONE
		//	if (ent->client->ps.powerups[PW_YSALAMIRI])
		//	{
		//		return;
		//	}
			FireWeapon( ent, qfalse );
...
void ClientThink_real( gentity_t *ent ) {
	...
	//Jayden FOR SAFE-ZONE
	/*if (ent->client->ps.iamfriendly)
	{
		ent->client->ps.powerups[PW_YSALAMIRI] = level.time + 100;
		...
		if (ent->client->pers.cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK))
		{
...
#define	MIN_NPC_SPEED	16.0f
	...
	if ( ent->client->ps.duelInProgress ) {
		...
		if ( !duelAgainst || !duelAgainst->client || !duelAgainst->inuse || duelAgainst->client->ps.duelIndex != ent->s.number ) {
			...
			if (ent->duelFullForce) {
				...
				} Fucking no! Full Force duels shouldnt have all Force Powers, idiots! */
	...
	if ( japp_flipKick.integer && client->ps.forceKickFlip ) {
			...
			&& (!ent->client->ps.duelInProgress || ent->client->ps.duelIndex == faceKicked->s.number) )
		{
				...
				&& !(faceKicked->client->ps.eFlags & EF_INVULNERABLE) )
			{
				...
				if ( japp_flipKickDamage.integer /*Jayden*/ && (!ent->client->ps.duelInProgress || ent->duelFullForce )) {
					G_Damage(
	...
	*/
	...
	//Анимации форс-абилок (Рейдж, Протект). Отключение анимки, если персонаж начал двигаться
	if ((client->pers.cmd.upmove != 0) || (client->pers.cmd.rightmove != 0) || (client->pers.cmd.forwardmove != 0))
	{
		if ((client->ps.torsoAnim == BOTH_FORCE_RAGE) && (client->ps.legsAnim == BOTH_FORCE_RAGE))
		{
			client->ps.torsoTimer = 0;
			client->ps.legsTimer = 0;
		}
		if ((client->ps.torsoAnim == BOTH_FORCE_ABSORB) && (client->ps.legsAnim == BOTH_FORCE_ABSORB))
		{
			client->ps.torsoTimer = 0;
			client->ps.legsTimer = 0;
		}
		if ((client->ps.torsoAnim == BOTH_FORCE_PROTECT) && (client->ps.legsAnim == BOTH_FORCE_PROTECT))
		{
			client->ps.torsoTimer = 0;
			client->ps.legsTimer = 0;
		}
		// Это пригодится, когда я сделаю анимацию печатания на консоли
		//Печатаем на консоли
		if ((client->ps.torsoAnim == BOTH_CONSOLE1) && (client->ps.legsAnim == BOTH_CONSOLE1))
		{
			client->ps.torsoTimer = 0;
			client->ps.legsTimer = 0;
		}
		//Уже нет.
		if ((client->ps.torsoAnim == BOTH_FORCEHEAL_START) && (client->ps.legsAnim == BOTH_FORCEHEAL_START))
		{
			client->ps.torsoTimer = 0;
			client->ps.legsTimer = 0;
		}	
	...
	//Конец куска про анимации форс-абилок JAYDEN
	if (level.time >= ent->attached_missle.StartTimerForZMissle + ent->attached_missle.ZMissleTimeDelay && ent->attached_missle.StartTimerForZMissle != -1)
	{
		//if (ent->client->ps.torsoAnim == ent->attached_missle.animation && ent->attached_missle.animation != 0)
		//{
			Z_Missle_spawn(ent);
		//}
