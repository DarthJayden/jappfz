...
static void AM_Clip( gentity_t *ent ) {
	...
	if (trap->Argc() > 1) //if (trap->Argc() != 2)
	{
		trap->Print("Usage: /amclip ID. Type just /amclip to noclip yourself.\n");
	}
	if (trap->Argc() != 2) //vas < 1
	{
	...
	else
	{
		...
		G_LogPrintf(level.log.admin, "\tturning %s for %s\n", target->client->noclip ? "on" : "off",
			G_PrintClient(target - g_entities));
		AM_ConsolePrint(target, va("Noclip %s\n", target->client->noclip ? S_COLOR_GREEN "on" : S_COLOR_RED "off"));
	}
}
...
void AM_Sethp(gentity_t *ent)
{
	...
	if (trap->Argc() < 3) {
		AM_ConsolePrint(ent, "Please specify a player and amount of health\n");
		return;
	}
	...
	if (atoi(arg1) == -1) {
		...
		for (i = 0, e = g_entities; i < level.maxclients; i++, e++) {
			...
			e->health = e->client->ps.stats[STAT_HEALTH] = targetEnt->playerState->stats[STAT_MAX_HEALTH] = e->client->pers.maxHealth = e->client->pers.newMaxHealth = atoi(arg2);
	...
	targetEnt->health = targetEnt->client->ps.stats[STAT_HEALTH] = targetEnt->client->pers.maxHealth = targetEnt->playerState->stats[STAT_MAX_HEALTH] = targetEnt->client->pers.newMaxHealth = atoi(arg2);
}

void AM_Setfp(gentity_t *ent)
{
	char arg1[64] = {};
	char arg2[64] = {};
	int targetClient;
	...
	if (trap->Argc() < 3) {
		AM_ConsolePrint(ent, "Please specify a player and amount of forcepoints\n");
		return;
	}
	...
	if (atoi(arg1) == -1) {
		...
		for (i = 0, e = g_entities; i < level.maxclients; i++, e++) {
			...
			e->playerState->fd.forcePower = e->playerState->fd.forcePowerMax = e->client->pers.newMaxForce = atoi(arg2);
	...
	targetEnt->playerState->fd.forcePower = targetEnt->playerState->fd.forcePowerMax = targetEnt->client->pers.newMaxForce = atoi(arg2);
}

void AM_Setar(gentity_t *ent)
{
	char arg1[64] = {};
	char arg2[64] = {};
	int targetClient;
	...
	if (trap->Argc() < 3) {
		AM_ConsolePrint(ent, "Please specify a player and amount of armor\n");
		return;
	}
	...
	if (atoi(arg1) == -1) {
		...
		for (i = 0, e = g_entities; i < level.maxclients; i++, e++) {
			...
			e->playerState->stats[STAT_ARMOR] = e->client->pers.newMaxArmor = atoi(arg2);
	...
	targetEnt->playerState->stats[STAT_ARMOR] = targetEnt->client->pers.newMaxArmor = atoi(arg2);
}

void Z_Missle_Think(gentity_t *ent)
{
	gentity_t *attach = &g_entities[ent->attached];

	if (attach->s.event & EV_MISSILE_HIT)
	{
		G_PlayEffectID(G_EffectIndex(ent->attached_missle.collision_effect), &ent->s.origin, &ent->s.angles);
		G_FreeEntity(ent);
		return;
	}
	if (attach->s.number == 0)
	{
		//G_PlayEffectID(G_EffectIndex(ent->attached_missle.collision_effect), &ent->s.origin, &ent->s.angles);
		G_FreeEntity(ent);
		return;
	...
	float X = attach->r.currentOrigin.x - ent->r.currentOrigin.x;
	float Y = attach->r.currentOrigin.y - ent->r.currentOrigin.y;
	float Z = attach->r.currentOrigin.z - ent->r.currentOrigin.z;
	ent->s.pos.trBase.x += X;
	ent->s.pos.trBase.y += Y;
	ent->s.pos.trBase.z += Z;
	ent->s.pos.trType = TR_LINEAR_STOP;
	ent->s.pos.trDuration = 1;  // move for 10secs unless stopped
...
void AM_Missle(gentity_t *ent, int velocity, int damage, const char *effect, const char *proj_class, const char *start_bone, int sizex, int sizey, int sizez, int cost, int bounce, int lifetime, int splash_radius, int splash_damage, const char *coll_effect, int handextend, int anim, int time, int xangle, int yangle, int zangle)
{
	ent->attached_missle.ZMissleTimeDelay = time;
	if (ent->attached_missle.StartTimerForZMissle == -1)
	{
		if (anim)
		{
			G_SetAnim(ent, NULL, SETANIM_BOTH, anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD | SETANIM_FLAG_HOLDLESS, 0); //BOTH_FORCEHEAL_START for example.
		}
		else
		{
			ent->client->ps.forceHandExtend = handextend; //HANDEXTEND_FORCEPUSH 4 example
			ent->client->ps.forceHandExtendTime = level.time + 1000;
		}

		ent->attached_missle.StartTimerForZMissle = level.time;
		ent->attached_missle.animation = anim; 
		ent->attached_missle.sizex = sizex;
		ent->attached_missle.sizey = sizey;
		ent->attached_missle.sizez = sizez;

		strncpy(ent->attached_missle.start_bone, start_bone, 64); 

		strncpy(ent->attached_missle.proj_class, proj_class, 64);

		ent->attached_missle.damage = damage; 

		ent->attached_missle.bounce = bounce;

		ent->attached_missle.splash_damage = splash_damage;

		ent->attached_missle.splash_radius = splash_radius;
		strncpy(ent->attached_missle.effect, effect, 64);


		xangle = ent->client->ps.viewangles.x + xangle;
		yangle = ent->client->ps.viewangles.y + yangle;
		zangle = ent->client->ps.viewangles.z + zangle;

		ent->attached_missle.xangle = xangle;
		ent->attached_missle.yangle = yangle;
		ent->attached_missle.zangle = zangle;
		ent->attached_missle.velocity = velocity; 
		ent->attached_missle.cost = cost; 
		ent->attached_missle.lifetime = lifetime; 

		int j = 0;
		for (j = 0; j < 64; j++)
		{
			ent->attached_missle.collision_effect[j] = coll_effect[j];
		}
	}
}

void Z_Missle_spawn(gentity_t *ent)
{
	vector3	forward;
	AngleVectors(&ent->client->ps.viewangles, &forward, NULL, NULL);
	mdxaBone_t boltMatrix;
	int bolt = trap->G2API_AddBolt(ent->ghoul2, 0, ent->attached_missle.start_bone);
	trap->G2API_GetBoltMatrix(ent->ghoul2, 0, bolt, &boltMatrix, &ent->client->ps.viewangles, &ent->client->ps.origin, level.time, NULL, &ent->modelScale);

	vector3 start;
	start.x = boltMatrix.matrix[0][3];
	start.y = boltMatrix.matrix[1][3];
	start.z = boltMatrix.matrix[2][3];

	gentity_t *missile = CreateMissile(&start, &forward, ent->attached_missle.velocity, ent->attached_missle.lifetime, ent, qfalse);

	missile->classname = (const char*)ent->attached_missle.proj_class;
	missile->s.weapon = 0;

	VectorSet(&missile->r.maxs, ent->attached_missle.sizex, ent->attached_missle.sizey, ent->attached_missle.sizez);
	//VectorSet(&missile->r.maxs, 300, 300, 300);
	VectorScale(&missile->r.maxs, -1, &missile->r.mins);

	missile->damage = ent->attached_missle.damage;
	missile->dflags = /*DAMAGE_NO_DAMAGE;*/DAMAGE_RADIUS;//DAMAGE_DEATH_KNOCKBACK;
	missile->splashMethodOfDeath = MOD_FORCE_DARK;
	missile->methodOfDeath = MOD_FORCE_DARK;
	missile->clipmask = MASK_SHOT;

	missile->flags |= FL_BOUNCE | DAMAGE_EXTRA_KNOCKBACK;
	missile->bounceCount = ent->attached_missle.bounce;
	missile->splashDamage = ent->attached_missle.splash_damage;
	missile->splashRadius = ent->attached_missle.splash_radius;

	gentity_t *obj = G_Spawn();
	VectorCopy(&missile->r.currentOrigin, &missile->s.origin);
	VectorCopy(&missile->s.origin, &obj->s.origin);
	obj->s.modelindex = G_EffectIndex(ent->attached_missle.effect);

	obj->classname = "fx_runner";
	// move editor origin to pos
	VectorCopy(&obj->s.origin, &obj->s.pos.trBase);
	VectorCopy(&obj->s.origin, &obj->r.currentOrigin);

	VectorSet(&obj->r.mins, -1 * 3, -1 * 3, -1 * 3);
	VectorSet(&obj->r.maxs, 3, 3, 3);
	...
	VectorSet(&obj->s.angles, ent->attached_missle.xangle, ent->attached_missle.yangle, ent->attached_missle.zangle);
	...
	obj->delay = 0; //atoi(buffer);
	obj->s.speed = 0; // obj->delay;

	obj->s.time = 0;// obj->random;
	...
	obj->attached = missile->s.number; //Ёффект прив€зываетс€ к энтити-снар€ду через индекс! 
	int j;
	for (j = 0; j < 64; j++)
	{
		obj->attached_missle.collision_effect[j] = ent->attached_missle.collision_effect[j];
	}
	...
	obj->think = Z_Missle_Think;
	trap->LinkEntity((sharedEntity_t *)obj);
	ent->attached_missle.StartTimerForZMissle = -1;
}
void AM_MissleID(gentity_t *ent)
{
	...
	int cost;
	int bounce;
	int splash_damage, splash_radius;
	int lifetime;
	int handextend, anim;
	int time; 
	int xangles;
	int yangles;
	int zangles;
	trap->Argv(1, arg0, sizeof(arg0));
	trap->Argv(2, arg1, sizeof(arg1));
	trap->Argv(3, arg2, sizeof(arg2));
	trap->Argv(4, effect, sizeof(effect));
	trap->Argv(5, proj_class, sizeof(proj_class));
	trap->Argv(6, start_bone, sizeof(start_bone));
	trap->Argv(7, arg6, sizeof(arg6));
	trap->Argv(8, arg7, sizeof(arg7));
	trap->Argv(9, arg8, sizeof(arg8));
	trap->Argv(10, arg9, sizeof(arg9));
	trap->Argv(11, arg10, sizeof(arg10));
	trap->Argv(12, arg11, sizeof(arg11));
	trap->Argv(13, arg12, sizeof(arg12));
	trap->Argv(14, arg13, sizeof(arg13));
	trap->Argv(15, coll_effect, sizeof(coll_effect));
	trap->Argv(16, arg15, sizeof(arg15));
	trap->Argv(17, animation, sizeof(animation));
	trap->Argv(18, arg18, sizeof(arg18));
	trap->Argv(19, arg19, sizeof(arg19));
	trap->Argv(20, arg20, sizeof(arg20));
	trap->Argv(21, arg21, sizeof(arg21));

	ID = atoi(arg0);
	velocity = (float)atoi(arg1);
	damage = atoi(arg2);
	sizex = atoi(arg6);
	sizey = atoi(arg7);
	sizez = atoi(arg8);
	...
	bounce = atoi(arg10);
	lifetime = atoi(arg11);
	splash_radius = atoi(arg12);
	splash_damage = atoi(arg13);
	handextend = atoi(arg15);
	anim = atoi(animation);
	time = atoi(arg18);
	xangles = atoi(arg19);
	yangles = atoi(arg20);
	...
	//Z_Missle_anim(player, handextend, anim, time);
	AM_Missle(player, velocity, damage, effect, proj_class, start_bone, sizex, sizey, sizez, cost, bounce, lifetime, splash_radius, splash_damage, coll_effect, handextend, anim, time, xangles, yangles, zangles);
	//Z_Missle_spawn(player);
	//player->attached_missle.StartTimerForZMissle = -1; 
}

void AM_Friend(gentity_t *ent)
{
	int clientN;
	if (trap->Argc() != 2)
	{
		clientN = ent->client->ps.clientNum; 
	}
	else
	{
		char arg1[64];
	...
	gentity_t *targ = &g_entities[clientN];
	if (targ->client->ps.iamfriendly == qtrue)
	{
		trap->SendServerCommand(clientN, "print\"You are no longer friendly\n\"");
		targ->client->ps.iamfriendly = qfalse;
		targ->flags &= ~FL_GODMODE;
		targ->flags &= ~FL_NO_KNOCKBACK;
	}
	else
	{
		trap->SendServerCommand(clientN, "print\"You are friendly\n\"");
		targ->client->ps.iamfriendly = qtrue;
		targ->flags |= FL_GODMODE;
		targ->flags |= FL_NO_KNOCKBACK;
	}
}
void AM_Unforced(gentity_t *ent)
{
	int clientN;
	if (trap->Argc() != 2)
	{
	...
	gentity_t *targ = &g_entities[clientN];
	if (targ->client->ps.iamunforcer == qtrue)
	{
		trap->SendServerCommand(clientN, "print\"You are no longer force-protected\n\"");
		targ->client->ps.iamunforcer = qfalse;
	}
	else
	{
		trap->SendServerCommand(clientN, "print\"You are force-protected\n\"");
		targ->client->ps.iamunforcer = qtrue;
	}
}
...
int Get_Num_For_AM_Give(const char arg2[64], const char arg3[64])
{
	//sizeof(arg2);
	//sizeof(arg3);
	if (!Q_stricmp(arg2, "weapon"))
	{
		if (!Q_stricmp(arg3, "all"))
		{
			 return -1;
		}
		else if (!Q_stricmp(arg3, "stun_baton"))
		{
			 return 1;
		}
		else if (!Q_stricmp(arg3, "melee"))
		{
			 return 2;
		}
		else if (!Q_stricmp(arg3, "saber"))
		{
			 return 3;
		}
		else if (!Q_stricmp(arg3, "pistol"))
		{
			 return 4;
		}
		else if (!Q_stricmp(arg3, "blaster"))
		{
			 return 5;
		}
		else if (!Q_stricmp(arg3, "disruptor"))
		{
			 return 6;
		}
		else if (!Q_stricmp(arg3, "bowcaster"))
		{
			 return 7;
		}
		else if (!Q_stricmp(arg3, "repeater"))
		{
			 return 8;
		}
		else if (!Q_stricmp(arg3, "demp2"))
		{
			 return 9;
		}
		else if (!Q_stricmp(arg3, "flechette"))
		{
			 return 10;
		}
		else if (!Q_stricmp(arg3, "rocket_launcher"))
		{
			 return 11;
		}
		else if (!Q_stricmp(arg3, "thermal"))
		{
			 return 12;
		}
		else if (!Q_stricmp(arg3, "trip_mine"))
		{
			 return 13;
		}
		else if (!Q_stricmp(arg3, "det_pack"))
		{
			 return 14;
		}
		else if (!Q_stricmp(arg3, "concussion"))
		{
			 return 15;
		}
		else if (!Q_stricmp(arg3, "bryar"))
		{
			 return 16;
		}
		else if (!Q_stricmp(arg3, "emplaced"))
		{
			 return 17;
		}
		else if (!Q_stricmp(arg3, "turret"))
		{
			 return 18;
		}
		else
	...
	else if (!Q_stricmp(arg2, "ammo"))
	{
		...
		else if (!Q_stricmp(arg3, "powercell"))
		{
			 return 3;
		}

		else if (!Q_stricmp(arg3, "metal_bolts"))
		{
			 return 4;
		}

		else if (!Q_stricmp(arg3, "rockets"))
		{
			 return 5;
		}
		...
		else if (!Q_stricmp(arg3, "tripmine"))
		{
			 return 8;
		}
		else if (!Q_stricmp(arg3, "detpack"))
		{
			 return 9;
		}
		else
	...
	else if (!Q_stricmp(arg2, "force"))
	{
		...
		else if (!Q_stricmp(arg3, "levitation"))
		{
			 return 1;
		}

		else if (!Q_stricmp(arg3, "speed"))
		{
			 return 2;
		}

		else if (!Q_stricmp(arg3, "push"))
		{
			 return 3;
		}

		else if (!Q_stricmp(arg3, "pull"))
		{
			 return 4;
		}

		else if (!Q_stricmp(arg3, "telepathy"))
		{
			 return 5;
		}

		else if (!Q_stricmp(arg3, "grip"))
		{
			 return 6;
		}

		else if (!Q_stricmp(arg3, "lightning"))
		{
			 return 7;
		}

		else if (!Q_stricmp(arg3, "rage"))
		{
			 return 8;
		}

		else if (!Q_stricmp(arg3, "protect"))
		{
			 return 9;
		}

		else if (!Q_stricmp(arg3, "absorb"))
		{
			 return 10;
		}

		else if (!Q_stricmp(arg3, "team_heal"))
		{
			 return 11;
		}

		else if (!Q_stricmp(arg3, "team_force"))
		{
			 return 12;
		}

		else if (!Q_stricmp(arg3, "drain"))
		{
			 return 13;
		}

		else if (!Q_stricmp(arg3, "see"))
		{
			 return 14;
		}

		else if (!Q_stricmp(arg3, "saber_offense"))
		{
			 return 15;
		}

		else if (!Q_stricmp(arg3, "saber_deffense"))
		{
			 return 16;
		}

		else if (!Q_stricmp(arg3, "saber_throw"))
		{
			 return 17;
		}

		else if (!Q_stricmp(arg3, "heal"))
		{
			 return 0;
		}
	}
	else if (!Q_stricmp(arg2, "item"))
	{
		if (!Q_stricmp(arg3, "all"))
		{
			 return -1;
		}
		else if (!Q_stricmp(arg3, "seeker"))
		{
			 return 1;
		}

		else if (!Q_stricmp(arg3, "shield"))
		{
			 return 2;
		}

		else if (!Q_stricmp(arg3, "medpack"))
		{
			 return 3;
		}

		else if (!Q_stricmp(arg3, "medpack_big"))
		{
			 return 4;
		}

		else if (!Q_stricmp(arg3, "binoculars"))
		{
			 return 5;
		}

		else if (!Q_stricmp(arg3, "sentry_gun"))
		{
			 return 6;
		}

		else if (!Q_stricmp(arg3, "jetpack"))
		{
			 return 7;
		}

		else if (!Q_stricmp(arg3, "healthdisp"))
		{
			 return 8;
		}

		else if (!Q_stricmp(arg3, "ammodisp"))
		{
			 return 9;
		}

		else if (!Q_stricmp(arg3, "eweb"))
		{
			 return 10;
		}

		else if (!Q_stricmp(arg3, "cloak"))
		{
			 return 11;
		}
		else
...
// TODO: Admin message. Jayden: will do with lua
static void AM_Give(gentity_t *ent){
	...
	if (trap->Argc() < 3) {
		AM_ConsolePrint(ent, "Syntax: \\amgive <client> ammo (like powercells)/weapon (like pistol or bowcaster) /force (like levitation or telepathy) and <amount> (0 takes it off!) \n");
		return;
	}
	...
	int num = Get_Num_For_AM_Give(arg2, arg3); //this way is broken somehow...
	//int num = Get_Pistol_Num_For_Am_Give(ent, arg3);
	...
	if (!Q_stricmp(arg2, "weapon")){
		...
		target->client->ps.stats[STAT_WEAPONS] |= (1 << num);
		//Jayden...
		if (atoi(arg4) == 0)
		{
			target->client->ps.stats[STAT_WEAPONS] &= ~(1 << num);
		...
		if (num == -1){
			for (int i = 0; i < NUM_FORCE_POWERS; i++) {
				target->client->ps.fd.forcePowerLevel[i] = atoi(arg4);// FORCE_LEVEL_3;
				target->client->ps.fd.forcePowersKnown |= (1 << i);
			}
			return;
		}
		if (num < FP_FIRST || num >= NUM_FORCE_POWERS){
			return;
		}
		target->client->ps.fd.forcePowerLevel[num] = atoi(arg4);
		target->client->ps.fd.forcePowersKnown |= (1 << num);
		...
		if (num <= AMMO_NONE || num >= AMMO_MAX){
			return;
		}
		if (arg4[0]){
			target->client->ps.ammo[num] = atoi(arg4);
		}
		else{
			target->client->ps.ammo[num] = 999;
		}
	...
	{
		if (num == -1) {
			target->client->ps.stats[STAT_HOLDABLE_ITEMS] = ((1 << HI_NUM_HOLDABLE) - 1) & ~1;
		}
		if (num <= HI_NONE || num >= HI_NUM_HOLDABLE) {
			return;
		}

		target->client->ps.stats[STAT_HOLDABLE_ITEMS] |= (1 << num);
		//Jayden...
		if (atoi(arg4) == 0)
		{
			target->client->ps.stats[STAT_HOLDABLE_ITEMS] &= ~(1 << num);
...
static const adminCommand_t adminCommands[] = {
	...
	//{ "notarget", PRIV_GHOST, AM_NoTarget },
	//{ "npc", PRIV_NPCSPAWN, Cmd_NPC_f },
	{ "setar", PRIV_SLAY, AM_Setar }, //set ar
	{ "setfp", PRIV_SLAY, AM_Setfp }, //set fpf
	{ "sethp", PRIV_SLAY, AM_Sethp }, //set hp
	{ "zmissle", PRIV_GOD, AM_MissleID},
	{ "zsafe", PRIV_GOD, AM_Friend},
	{ "zunforced", PRIV_GOD, AM_Unforced },
