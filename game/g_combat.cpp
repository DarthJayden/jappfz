...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, vector3 *dir, vector3 *point, int damage, uint32_t dflags, int mod ) {
	...
	if ( targ && targ->client ) {
		...
		if ((japp_chatProtection.integer && (targ->client->ps.eFlags & EF_TALK)) && !targ->client->ps.duelInProgress /*Jayden*/ && !targ->client->grappleState && !targ->client->damage_armor && !targ->client->damage_blood && !targ->client->ps.fd.forceGripBeingGripped/*/Jayden*/) {
			return;
		}
...
static qboolean CanDamage( gentity_t *targ, gentity_t *attacker, vector3 *origin ) {
	...
	if ( targ && attacker && targ->client ) {
		if ( japp_chatProtection.integer && (targ->client->ps.eFlags & EF_TALK) /*Jayden*/ && !targ->client->grappleState && !targ->client->damage_armor && !targ->client->damage_blood && !targ->client->ps.fd.forceGripBeingGripped/*/Jayden*/) {
			return qfalse;
		}
