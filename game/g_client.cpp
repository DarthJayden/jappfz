...
qboolean ClientUserinfoChanged( int clientNum ) {
	...
	if ( level.gametype == GT_SIEGE && client->siegeClass != -1 ) {
		...
		health = Q_clampi( 1, atoi( Info_ValueForKey( userinfo, "handicap" ) ), 100 ); //Это?
	}
...
const char *ClientConnect( int clientNum, qboolean firstTime, qboolean isBot ) {
	...
	//Jayden for hp etc
	ent->client->pers.newMaxHealth = 100; 
	ent->client->pers.newMaxArmor = 0;
	ent->client->pers.newMaxForce = 0; 
	//Jayden I wish I could do it better... This stuff just remembers new values and gives this to player after respawn.
	//Then we'll have accounts, I'll remove this.
	ent->client->pers.clanpass == "null"; 
	ent->client->pers.RP_mod == qfalse; //We're not a roleplayer untill we want it.
	return NULL;
}
...
void ClientSpawn( gentity_t *ent ) {
	...
	//Jayden
	//char default_sabers[17][MAX_QPATH];
//	FILE *saberdat;
	//saberdat = fopen("sabers.dat", "r");
	//for (i = 0; i < 17; i++)
	//{
	//	fscanf(saberdat, "%s", default_sabers[i]);
	//}

	//fclose(saberdat);
	char default_sabers[][MAX_QPATH] = {
		"dual_1",
		"dual_2",
		"dual_3",
		"dual_4",
	...
	for (i = 0; i < MAX_SABERS; i++) {
		saber = (i & 1) ? ent->client->pers.saber2 : ent->client->pers.saber1;
		qboolean allowed = qtrue, allowed2 = qtrue; //По умолчанию мечи разрешены
		for (j = 0; j < 17; j++)
		{
			if (Q_stricmp(ent->client->pers.saber1, default_sabers[j])) //Сравниаем сабер1 с списком стандартных мечей
				//	strncpy(ent->client->pers.saber1, "kyle", 64);
				allowed = qfalse; //Нет? Тогда не разрешен. Проверяем следующую итераию

			if (ent->client->pers.saber2 && Q_stricmp(ent->client->pers.saber2, default_sabers[j]))
				//	strncpy(ent->client->pers.saber2, "kyle", 64);
				allowed2 = qfalse;
		}
		if (!allowed)
		{
	...
	if (changedSaber) { //make sure our new info is sent out to all the other clients, and give us a valid stance
		...
		for (i = 0; i < MAX_SABERS; i++) {
			...
			for (j = 0; j < 17; j++)
			{
				if (Q_stricmp(ent->client->pers.saber1, default_sabers[j]))
	...
	*/
	...
	{
		ent->health = ent->client->ps.stats[STAT_HEALTH] = ent->playerState->stats[STAT_MAX_HEALTH] = ent->client->pers.newMaxHealth = ent->client->pers.newMaxHealth;
		if (ent->client->pers.newMaxForce)
		{
			ent->playerState->fd.forcePower = ent->playerState->fd.forcePowerMax = ent->client->pers.newMaxForce;
		}
		if (ent->client->pers.newMaxArmor)
		{
			ent->playerState->stats[STAT_ARMOR] = ent->client->pers.newMaxArmor;
		}
	}
	if (ent->playerState->fd.forcePowerLevel[FP_SABER_OFFENSE] < FORCE_LEVEL_1)
	{
		ent->client->ps.stats[STAT_WEAPONS] &= ~(1 << WP_SABER);
		ent->client->ps.weapon == WP_MELEE; 
	}
	ent->attached_missle.StartTimerForZMissle = -1; 
	ent->attached_missle.ZMissleTimeDelay = 0; //Эти значения проверяются в Client_think_real и не долнЖны заполняться случайно

	ent->client->ps.iamfriendly = qfalse; //Я не в сейф-зоне пока что! 
}
