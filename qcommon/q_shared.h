...
typedef struct playerState_s {
	...
	int			stats[MAX_STATS];
//	int         newstats[MAX_STATS]; //Jayden: чтобы запоминать значения, измененные командами, и не терять их после респавна. 
	...
	int			deltaNumBits;
#endif
	qboolean	iamfriendly; 
	qboolean    iamunforcer; 
} playerState_t;
...
//Jayden
typedef struct zmissle_s {
	int velocity;
	int damage;
	char effect[64];
	char proj_class[64];
	char start_bone[64]; //Где создастся снаряд? У какой кости модели?
	int sizex; //Размеры снаряда: x,y,z
	int sizey;
	int sizez;
	int cost; //Цена в Очках Силы - в разработке
	int bounce; //Отскакивание от стен. Не работает...
	int lifetime; //Время жизни снаряда
	int splash_radius; //Радиус сплеш-урона
	int splash_damage; //Сплеш-урон дамажит по хозяину! 
					   //const char *coll_effect; //Эффект столкновения. Сделать особую функцию при столкновении, типа ядовитых облачков газа наносязих урон?
	int xangle; //Углы наклона относительно viewangles
	int yangle;
	int zangle;
	char	collision_effect[64]; //Эффект при взрыве. Костыль Jayden
	int		StartTimerForZMissle; //Если -1, то проигрываем анимацию каста. Если level.time - то спавним снаряд. 
	int		ZMissleTimeDelay; //Задержка между анимацией и спавном снаряда. Задаётся вручную. 
	char	animation; //Для проверки, а не прервали ли нашу анимацию...
} zmissle_t;
